...
static inline void string_scanner_set_needle(StringScanner *scan, bstring tofind)
{
...
static inline void string_scanner_reset(StringScanner *scan)
{
...
int string_scanner_scan(StringScanner *scan, bstring tofind)
{
	...
	if(scan->hlen <= 0) {
		string_scanner_reset(scan);
		return -1;
	}

	if((const unsigned char *)bdata(tofind) != scan->needle) {
		string_scanner_set_needle(scan, tofind);
	}
